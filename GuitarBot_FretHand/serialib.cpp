...
void timeOut::initTimer()
{
    ...
    auto now = time_point_cast<milliseconds>(system_clock::now()).time_since_epoch();
    start = duration_cast<milliseconds>(now).count();
}
...
unsigned long int timeOut::elapsedTime_ms()
{
    using namespace std::chrono;
    auto now = time_point_cast<milliseconds>(system_clock::now()).time_since_epoch();
    return duration_cast<milliseconds>(now).count() - start;
}